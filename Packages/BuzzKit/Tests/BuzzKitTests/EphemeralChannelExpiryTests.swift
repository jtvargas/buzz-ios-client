...
/// (`buzz-relay/src/handlers/side_effects.rs:1101-1107`) — and then never deletes the
/// row. There is no `DELETE FROM channels` in the relay at all. So a channel asked to
/// live five minutes stays open, answerable and discoverable indefinitely, and open-channel
/// discovery shows it to everybody on that relay.
///
