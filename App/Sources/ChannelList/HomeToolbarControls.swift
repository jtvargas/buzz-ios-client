...
struct HomeToolbarControls: View {
    /// The history, already resolved against the live sidebar and the active community.
    ...
    private var historyButton: some View {
        ...
        .popover(isPresented: $showsHistory) {
            RecentPlacesPopover(places: places, names: names) { place in
                showsHistory = false
                openPlace(place)
            }
    ...
    /// The clock with the arrow running backwards around it. Named here rather than at the
    /// call site because a misspelt system symbol renders as *nothing* — no warning, no
    /// placeholder — and one constant is what a test can assert exists.
    static let symbol = "clock.arrow.trianglehead.counterclockwise.rotate.90"
